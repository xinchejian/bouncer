...
if ($macAddr) $mac2 = '"'.mysql_real_escape_string($macAddr, $link).'"';
mysql_query("INSERT IGNORE members.Users (email,since,password,salt,mac) VALUES($email2,NOW(),$password2,$salt2,$mac2)", $link)
	or mail_and_die('mysql_query INSERT Users error');
// FIXME: might fail because of unique password (change salt) or mac (ignore)
